...
export default function useLanguage(): string | null {
  ...
  const selectedLanguage =
    mapLanguageCadidateToSupportedLanguage(languageCandidate) ??
    DEFAULT_LANGUAGE
...
function determineLanguageFromQueryValue(
  ...
  if (!value) {
    ...
    if (CAN_ACCESS_COOKIES) {
      ...
      previouslySelectedLanguage = getValueFromCookieString(
        document.cookie,
        LANGUAGE_COOKIE_KEY,
      )
    }
    ...
    if (previouslySelectedLanguage) {
      const supportedLanguage = mapLanguageCadidateToSupportedLanguage(
